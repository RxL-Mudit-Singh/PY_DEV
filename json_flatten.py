...
def _construct_key(previous_key, separator, new_key, replace_separators=None):
    ...
    if previous_key:
        ...
        return new_key
def flatten(nested_dict,separator=".", root_keys_to_ignore=None,replace_separators=None):
    """
...
def flatten_preserve_lists(nested_dict, separator=".", root_keys_to_ignore=None, max_list_index=3, max_depth=3, replace_separators=None):
    """
    Flattens a dictionary with nested structure to a dictionary with no
    hierarchy
    Consider ignoring keys that you are not interested in to prevent
